...
public class RegalosServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(request.getParameter("ws").equals("1")){
			...
			LinkedList<Regalo> lista = (LinkedList<Regalo>) regalosService.getRegalosPorEdad();
			JSONArray wrapper = new JSONArray();
			Regalo regaloInventado = new Regalo(54, "Exprimidor", "Exprime tus jugos",
					25.0); 
			
			JSONObject regalo = new JSONObject();
			regalo.put("id", regaloInventado.getId());
			regalo.put("name", regaloInventado.getName());
			regalo.put("description", regaloInventado.getDescription());
			regalo.put("price", regaloInventado.getPrice());
			wrapper.add(regalo);
